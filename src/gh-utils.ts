export class GithubClient {
  ...
  repos(user: string = "", org: string = "") {
    ...
    let pathPrefix = user ? `users/${user}` : org ? `orgs/${org}` : "user";
  ...
  async dirlist(
    ...
    for (let i = 0; i < pathElems.length; i++) {
      ...
      let found = _dirList
        ? _dirList.tree?.find((item: any) => item.path === pathElems[i])
        : null;
    ...
    if (url) {
      let resp = await fetch(url, { headers });
      let _dirList: any = resp.ok ? await resp.json() : {};
      files = (_dirList.tree || []).map((item: any) => ({
        name: item.path,
        sha: item.sha,
  ...
  async fullPath(
    ...
    if (!foldersOnly) {
      ...
      for (let i = 0; i < toFind.length; i++) {
        if (
          dirList.find(
            (item) => item.type === "file" && item.name === toFind[i],
          )
        ) {
