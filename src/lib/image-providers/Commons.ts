...
export class WikimediaCommons extends ImageProvider {
  ...
  async getDepicts() {
    ...
    this._images.forEach((img:Image) => {
      if (!img.depicts || !img.depicts.length) img.depicts = [{id: this._qid}]
  ...
  async imageSelected(image: Image): Promise<string[]> {
    return fetch(`https://commons.wikimedia.org/wiki/Special:EntityData/M${image.pageid}.json`)
      .then(resp => resp.json())
      .then(resp => {
        let entity = resp.entities[`M${image.pageid}`]
        let depicts = entity.statements.P180?.map((item:any) => item.mainsnak.datavalue.value.id) || []
  ...
  async _doQuery() {
    ...
    let images = [...sparqlResponse, ...apiResponse.filter((img:Image) => !sparalImageIds.includes(img.id))]
    this._images = images
