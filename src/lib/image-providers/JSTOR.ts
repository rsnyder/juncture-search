...
export class JSTOR extends ImageProvider {
  ...
  name: string = "JSTOR";
  static logo: string =
    "https://about.jstor.org/wp-content/themes/aboutjstor2017/static/JSTOR_Logo2017_90.png";
  ...
  async _doQuery() {
    let label =
      this._entity.label.indexOf(" ") > 0
        ? `"${this._entity.label}"^2`
        : `${this._entity.label}^2`;
    let aliases = this._entity.aliases.map((alias: string) =>
      alias.indexOf(" ") > 0 ? `"${alias}"^1` : `${alias}^1`,
    );
    ...
    if (resp.ok) {
      ...
      let updated: any = [
        ...(this._images || []),
        ...results.items.map((item: any) => this._transformItem(item)),
      ];
  ...
  _transformItem(item: any): any {
    ...
    doc.url = `https://www.jstor.org/stable/${
      item.doi.indexOf("10.2307") === 0 ? item.doi.slice(8) : item.doi
    }`;
    ...
    if (item.primary_agents?.length > 0)
      doc.creator = item.primary_agents.join("; ");
    let ccLicense = item.cc_reuse_license[0]?.toLowerCase() || "";
    if (ccLicense.indexOf("public domain mark") >= 0)
      doc.license = "https://creativecommons.org/publicdomain/mark/1.0";
    else if (ccLicense.indexOf("creative commons: free reuse (cc0)") >= 0)
      doc.license = "https://creativecommons.org/publicdomain/zero/1.0";
    else if (
      ccLicense.indexOf(
        "creative commons: attribution-noncommercial-noderivs",
      ) >= 0
    )
      doc.license = "CC BY-NC-ND";
    else if (
      ccLicense.indexOf(
        "creative commons: attribution-noncommercial-sharealike",
      ) >= 0
    )
      doc.license = "CC BY-NC-SA";
    else if (
      ccLicense.indexOf("creative commons: attribution-noncommercial") >= 0
    )
      doc.license = "CC BY-NC";
    else if (ccLicense.indexOf("creative commons: attribution-noderivs") >= 0)
      doc.license = "CC BY-ND";
    else if (ccLicense.indexOf("creative commons: attribution-sharealike") >= 0)
      doc.license = "https://creativecommons.org/licenses/by-sa/4.0";
    else if (ccLicense.indexOf("creative commons: attribution") >= 0)
      doc.license = "https://creativecommons.org/licenses/by/4.0";
