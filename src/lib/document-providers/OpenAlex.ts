...
export class OpenAlex extends ArticleProviderBase {
  ...
  logo: string =
    "https://www.gitbook.com/cdn-cgi/image/width=24,dpr=2,height=24,fit=contain,format=auto/https%3A%2F%2F2520693015-files.gitbook.io%2F~%2Ffiles%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FpHVuV3Ib5KXeBKft4Kcl%252Ficon%252FogQFSOcLuYxWvYBhmDaU%252Fopenalex%2520logo%2520twitter.jpg%3Falt%3Dmedia%26token%3D5136a792-47d6-4d18-96eb-2012b55844d0";
  ...
  async _doQuery() {
    ...
    let qargs = Object.keys(searchArgs)
      .map((k) => `${k}=${searchArgs[k]}`)
      .join("&");
  ...
  _transformItem(item: any): any {
    ...
    if (item.publication_date)
      article.publication_date = new Date(item.publication_date);
    ...
    if (item.primary_location?.source)
      article.publications = [
        { label: item.primary_location.source.display_name },
      ];
    if (item.concepts)
      article.main_subjects = item.concepts.map((c) => ({
        id: c.wikidata.split("/").pop(),
