...
export class Wikidata extends ArticleProviderBase {
  ...
  async next(howMany: number = 50): Promise<Article[]> {
    ...
    if (this._details.length < this._index.length) {
      let qids = this._index
        .slice(this._details.length, this._details.length + howMany)
        .map((item) => item.id);
  ...
  async _getIndex() {
    ...
    if (resp.ok) {
      ...
      _resp.results.bindings.map((b) => {
        ...
        if (!data[id])
          data[id] = { id, mainSubject: [], pubdate: b.pubdate?.value };
        if (b.mainSubject)
          data[id].mainSubject.push({
            id: b.mainSubject.value.split("/").pop(),
            label: b.mainSubjectLabel.value,
          });
      });
  ...
  async _getDetails(qids: string[]) {
    let query = this.ITEMS_SPARQL.replace(
      /{{qids}}/g,
    ...
    await fetch(
      ...
      .then((resp) => resp.json())
      .then((jld) =>
        jsonld.frame(jld, { "@context": this._context, "@type": "Entity" }),
      )
      .then((jld) => {
  ...
  _transformEntity(e: any): Article {
    ...
    if (e.main_subjects)
      article.main_subjects = Array.isArray(e.main_subjects)
        ? e.main_subjects.map((ms) => ({
            id: ms.main_subjects["@id"].replace(/wd:/, ""),
            label: ms.label,
          }))
        : [
            {
              id: e.main_subjects.main_subjects["@id"].replace(/wd:/, ""),
              label: e.main_subjects.label,
            },
          ];
    if (e.pages) article.volume = e.pages;
    if (e.publication)
      article.publications = Array.isArray(e.publication)
        ? e.publication.map((p) => ({
            id: p.publication["@id"].replace(/wd:/, ""),
            label: p.label,
          }))
        : [
            {
              id: e.publication.publication["@id"].replace(/wd:/, ""),
              label: e.publication.label,
            },
          ];
    if (e.publication_date)
      article.publication_date = new Date(e.publication_date);
    ...
    article.authors = [
      ...(e.author_name_string
        ? Array.isArray(e.author_name_string)
          ? e.author_name_string.map((ans) => ({
              label: ans.author_name_string,
              seq: parseInt(ans.ordinal),
            }))
          : [
              {
                label: e.author_name_string.author_name_string,
                seq: parseInt(e.author_name_string.ordinal),
              },
            ]
        : []),
      ...(e.author
        ? Array.isArray(e.author)
          ? e.author.map((a) => ({
              id: a.author["@id"].replace(/wd:/, ""),
              label: a.label,
              seq: parseInt(a.ordinal),
            }))
          : [
              {
                id: e.author.author["@id"].replace(/wd:/, ""),
                label: e.author.label,
                seq: parseInt(e.author.ordinal),
              },
            ]
        : []),
    ]
      .sort((a, b) => a.seq - b.seq)
      .map((a) => delete a.seq && a);
