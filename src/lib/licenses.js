...
let licensesByQID = Object.fromEntries(
  Object.entries(licenses).map(([url, value]) => [
    value.qid,
    { ...value, ...{ url } },
  ]),
);
...
let licensesByCode = Object.fromEntries(
  Object.entries(licenses).map(([url, value]) => [
    value.code,
    { ...value, ...{ url } },
  ]),
...
function licenseUrl(licenseStr) {
  ...
  if (licenseStr.indexOf("http") === 0) {
    ...
    // licenseStr is a code
    licenseStr = licenseStr.toUpperCase().replace(/\s+/g, " ");
    if (licensesByCode[licenseStr]) licenseUrl = licensesByCode[licenseStr].url;
    else if (licenseStr.indexOf("BY") === 0)
      licenseUrl = licensesByQID["Q20007257"].url;
    else if (licenseStr.indexOf("BY-SA") === 0)
      licenseUrl = licensesByQID["Q18199165"].url;
  }
