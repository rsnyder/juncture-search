...
export const useEntitiesStore = defineStore("entities", {
  ...
  actions: {
    addEntities(entities: any[]) {
      ...
      this.entityData = {
        ...this.entityData,
        ...Object.fromEntries(entities.map((ent) => [ent.id, ent])),
      };
    ...
    setUser(user: any) {
      ...
      this.isLoggedIn =
        user !== null &&
        (user.provider === "github" || tokenIsValid(user.tokenExpiration));
      this.userid =
        user !== null ? sha256(user.email.toLowerCase().trim()) : null;
    },
      ...
      if (eid && !this.entityData[eid]) {
        ...
        try {
          ...
          findQids(result.entities[eid].claims).forEach((eid) => {
            ...
            if (eid[0] === "P" && !this.urlformatters[eid])
              this.urlformatters[eid] = undefined;
          });
    ...
    async updateLabels(qids: string[] = []) {
      // console.log(`updateLabels: qids=${qids.length}`)
      let before = Object.keys(this.allLabels).length;
      for (let qid of qids) {
        if (!this.allLabels[qid]) this.allLabels[qid] = {};
      }
      // console.log(`updateLabels: ${before} -> ${Object.keys(this.allLabels).length}`)
      if (!this.labelsFetching) {
        let toUpdate = Object.keys(this.allLabels).filter(
          (qid) => this.allLabels[qid][this.language] === undefined,
        );
        // console.log(`updateLabels: toUpdate=${toUpdate.length} language=${this.language}`)
        if (toUpdate.length > 0) {
          ...
          console.log(
            `updateLabels: ${toUpdate.length} -> ${
              Object.keys(this.allLabels).length
            }`,
          );
        }
    ...
    async updateUrlformatters() {
      if (!this.urlformattersFetching) {
        let qids = Object.keys(this.urlformatters).filter(
          (pid: string) => this.urlformatters[pid] === undefined,
        );
        // console.log(`updateUrlformatters: qids=${qids.length}`)
        if (qids.length > 0) {
          this.urlformattersFetching = true;
          let values = qids
            .map((pid) => `(<http://www.wikidata.org/entity/${pid}>)`)
            .join(" ");
          ...
          let updates = Object.fromEntries(
            sparqlResp.results.bindings.map((rec: any) => [
              rec.prop.value.split("/").pop(),
              rec.url.value,
            ]),
          );
          if (Object.keys(updates).length > 0)
            this.urlformatters = { ...this.urlformatters, ...updates };
          Object.keys(urlformatters).forEach(
            (pid) =>
              (urlformatters[pid] = urlformatters[pid] || updates[pid] || null),
          );
    ...
    async setEntityForLanguage(
      entity: any,
      language: string = "",
      addSummaryText: boolean = false,
    ) {
      ...
      if (entity.aliases && Object.keys(entity.aliases).length > 0)
        revised.aliases = (
          ...
          entity.aliases.en ||
          entity.aliases[Object.keys(entity.aliases)[0]]
        ).map((a: any) => a.value);
      ...
      if (entity.sitelinks && entity.sitelinks[`${language}wiki`]) {
        ...
        if (addSummaryText) {
          if (entity.summaryText[language]) {
            ...
            let page: number = entity.sitelinks[`${language}wiki`].url
              .replace(/\/w\//, "/wiki")
              .split("/wiki/")
              .pop();
            let resp: any = await fetch(
              `https://${language}.wikipedia.org/api/rest_v1/page/summary/${page}`,
            );
            if (resp.ok) {
              resp = await resp.json();
            }
            entity.summaryText[language] =
              resp["extract_html"] || resp["extract"];
            revised.summaryText = entity.summaryText[language];
          }
        }
      }
