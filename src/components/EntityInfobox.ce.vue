...
watch(shadowRoot, () => {
  new HSTooltip(shadowRoot.value).init();
});
...
const description = computed(
  () => entity.value?.descriptions?.[props.language]?.value,
);
const wikipediaLink = computed(
  () => entity.value && entity.value.sitelinks[`${props.language}wiki`]?.url,
);
...
<template>
  <div
    ref="root"
    class="hs-tooltip inline-block [--trigger:click] [--placement:top]"
  >
    <a
    ...
    >
      <span
        v-html="text"
        class="text-red-800 underline underline-offset-4 decoration-dotted"
      ></span>

      <div
        class="hs-tooltip-content z-30 hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible hidden opacity-0 transition-opacity absolute invisible max-w-xs bg-white border border-gray-100 text-left rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700"
        role="tooltip"
      >
        <div
        ...
        >
          ...
          <div class="p-4 md:p-5">
            ...
            <div
              class="flex items-center space-x-4 mt-5 text-xs text-gray-500 dark:text-gray-500"
            >
              <a
                class="inline-flex items-center gap-2 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm"
                target="_blank"
                :href="wikipediaLink"
              >
                <img
                  class="max-w-[24px] rounded-full"
                  src="https://upload.wikimedia.org/wikipedia/commons/7/77/Wikipedia_svg_logo.svg"
                />
                <!-- <span class="ml-1">Wikipedia</span> -->
              </a>
              <a
                class="inline-flex bg-[#444A1E] h-[24px] w-[24px] rounded-md items-center justify-center gap-2 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm"
                target="_blank"
                :href="`/entity/${qid}`"
              >
