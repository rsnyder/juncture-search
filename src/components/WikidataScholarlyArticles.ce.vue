<template>
  ...
  <div ref="root">
    <span v-html="props.label" class="title"></span> <span v-if="ids" class="count">({{ ids?.length.toLocaleString() }})</span>
    <ve-pal
      :id="id"
      :active="isActive"
      :articles="items" 
...
<script setup lang="ts">
  ...
  function getItems() {
    ...
    let query = ITEMS_SPARQL.replace(/{{qids}}/g, qids.map(q => `wd:${q}`).join(') (')).trim()
    ...
    .then(resp => resp.json())
    .then(jld => jsonld.frame(jld, {'@context': context, '@type': 'Entity'}))
    .then(jld => {
    ...
    .then(entities => {
      entities.map((entity: any) => {
        ...
        if (!Array.isArray(entity['main subject'])) entity['main subject'] = [entity['main subject']]
