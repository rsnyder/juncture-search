<template>
  <div ref="root">
    <div
      v-if="isLoading"
      class="flex w-full items-center justify-center space-x-4 mt-8"
    >
      <div
    ...
    >
      ...
      <div class="flex flex-col">
        <div class="flex items-center space-x-2">
          ...
          <div v-if="e.types" class="font-light text-sm">
            ({{ e.types.join(", ") }})
          </div>
        </div>
...
watch(entity, async () => {
  if (entities.value.length === 0) entities.value = await loadData();
});
  ...
  if (resp.ok) {
    ...
    Object.entries(typeData.categories).forEach(([category, values]) => {
      ...
      (<string[]>values).forEach((val: string) => {
        if ((val[0] === "Q" || val[0] === "P") && isNumeric(val.slice(1)))
          ids.add(val);
...
function typeIds(item: any): string[] {
  ...
  if (item.occupation)
    item.occupation.forEach((occupation: any) => {
  ...
  if (item.instance_of)
    item.instance_of.forEach((instance_of: any) => {
      if (typeof instance_of === "string") qids.add(instance_of.slice(3));
      else
        instance_of.instance_of
          .filter((arg: any) => typeof arg === "string")
          .forEach((qid: string) => qids.add(qid.slice(3)));
    });
  if (item.subclass_of)
    item.subclass_of.forEach((subclass_of: any) => {
  ...
  if (item.part_of)
    item.part_of.forEach((part_of: any) => {
      if (typeof part_of === "string") qids.add(part_of.slice(3));
        ...
        if (part_of.part_of)
          part_of.part_of
            .filter((arg: any) => typeof arg === "string")
            .forEach((qid: string) => qids.add(qid.slice(3)));
        if (part_of.subclass_of)
          part_of.subclass_of
            .filter((arg: any) => typeof arg === "string")
            .forEach((qid: string) => qids.add(qid.slice(3)));
        if (part_of.instance_of)
          part_of.instance_of
            .filter((arg: any) => typeof arg === "string")
            .forEach((qid: string) => qids.add(qid.slice(3)));
      }
...
function inferTypes(item: any, itemTypes: string[], seq: number) {
  ...
  Object.entries(byCategory.value).forEach(([category, obj]) => {
    if (setIntersection(itemTypes, (<any>obj).ids).size > 0) {
      ...
      if (
        Array.from((<any>obj).snippets).find(
          (snippet) =>
            item.description && item.description.indexOf(snippet) >= 0,
        )
      )
        types.add(category);
    }
  });
  let msg = `${seq}: ${item.label} (${item.description}) [${Array.from(
    types,
  )}]`;
  if (types.size === 0) {
    types.add("Other");
...
async function mentionedEntities() {
  ...
  let qids = Object.values(mentions)
    .filter((mention: any) => mention.qid)
    .map((mention: any) => mention.qid);
  if (qids.length > 0) {
    let query = entitiesSparqlTemplate
      .replace(/{{qids}}/, `(${qids.join(") (")})`)
      .replace(/{{language}}/g, language.value);
...
async function mentionTitles() {
  ...
  if (html) {
    let tmp = new DOMParser().parseFromString(html, "text/html").children[0]
      .children[1];
    let mentions: any = {};
    Array.from(tmp.querySelectorAll("a"))
      .filter(
        (link) => link.href.indexOf(`${window.location.origin}/wiki/`) === 0,
      )
      .filter(
        (link) =>
          link.href.indexOf("Special:") < 0 &&
          link.href.indexOf("File:") < 0 &&
          link.href.indexOf("Category:") < 0,
      )
      .map((link) => link.href.split("/").pop())
      .forEach(
        (title: any) =>
          (mentions[title] = { title, count: (mentions[title] || 0) + 1 }),
      );
    return mentions;
  }
}
...
async function mentionQids(mentions: any[]) {
  let pages = Object.keys(mentions)
    .map(
      (title) =>
        `<https://${language.value}.wikipedia.org/wiki/${encodeURIComponent(
          title,
        )}>`,
    )
    .join(" ");
...
async function mwUrl() {
  let query = mwUrlSparqlTemplate
    .replace(/{{qid}}/, entity.value.id)
    .replace(/{{language}}/g, language.value);
  let resp: any = await fetch(sparqlEndpoint, {
    method: "POST",
    body: new URLSearchParams({ query }),
