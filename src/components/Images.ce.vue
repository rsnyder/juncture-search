<template>
  ...
  <div ref="root">
    ...
    -->
    <ve-drawer class="w-full" :options="options" @options-updated="optionsUpdated">
      <ve-image-grid 
        :id="id"
        :active="isActive"
...
<script setup lang="ts">
  ...
  function setOptions() {
    options.value = {
      providers: 
        Array.from(imageProviders
          .filter(p => p.enabled)
          .reduce((acc, p) => { if (p.enabled) acc.add(p.tag); return acc }, new Set())
        )
        .map(tag => ({ label: tag, enabled: true, logo: imageProviders.find(p => p.tag === tag)?.class.logo }))
    }
  }
  ...
  async function getNext() {
    let isEnabled = (tag: string) => options.value.providers.find((p: any) => p.label === tag)?.enabled
