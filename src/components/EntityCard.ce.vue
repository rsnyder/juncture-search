<template>
  ...
  <div ref="root" class="max-w-xs rounded overflow-hidden shadow-lg" @click="onclick">
    <img 
...
<script setup lang="ts">
  ...
  const backgroundImage = computed(() => thumbnail.value && `url('${encodeUrl(thumbnail.value)}')`)
  ...
  function encodeUrl(url:string) {
    let parts = url.split('/')
    return `${parts.slice(0,-1).join('/')}/${encodeURIComponent(parts[parts.length-1])}`
  }
