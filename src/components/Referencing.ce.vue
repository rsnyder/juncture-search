          ...
          <template v-if="prop.values.length > 5">
            <div class="collapsible">
              ...
              <div class="prop-values-content">
                <div
                  v-for="(pval, idx) in prop.values"
                  :key="`prop-${prop.id}-${idx}`"
                >
                  <div v-if="prop.type === 'WikibaseItem'" class="prop-value">
                    <span
                      class="wikibase-entityid"
                      v-html="pval.value.label"
                      @click="entitySelected"
                      :data-qid="pval.value.id"
                      :id="`related-${prop.id}-${idx}-${pval.value.id}`"
                    >
                    </span>
                    <span
                      class="description"
                      v-html="pval.value.description"
                    ></span>
                  </div>
...
watch(isActive, () => {
  if (entity.value.id !== qid.value && isActive.value) loadData();
});
...
async function loadData() {
  ...
  if (resp.ok) {
    ...
    let entities = normalizeJsonld(
      await jsonld.frame(jld, {
        "@context": context,
        "@type": "Entity",
        options: { compactArrays: false },
      }),
    );
    let properties = normalizeJsonld(
      await jsonld.frame(jld, {
        "@context": context,
        "@type": "Property",
        options: { compactArrays: false },
    ...
    claims.value = asClaims(entities, properties).filter(
      (prop: any) => exclude.indexOf(prop.label) === -1,
    );
  }
}
...
function normalizeJsonld(jsonld: any) {
  ...
  Object.keys(jsonld)
    ...
    .forEach((key) => (normalized["@graph"][0][key] = jsonld[key]));
  return normalized;
}
