        ...
        >
          <ul>
            <li>
              ...
              <span v-if="item.aliases" class="aliases"
                >({{ item.aliases.join(", ") }})</span
              >
            </li>
            <li class="description" v-html="item.description"></li>
          </ul>
        </li>
        <li v-if="searchContinue" @click="doSearch" class="continue">
          More...
        </li>
      </ul>
    </div>
...
function init() {
  autocompleteResults.value = shadowRoot.value?.querySelector(
    "#autocompleteResults",
  ) as HTMLInputElement;
  autocompleteInput.value = shadowRoot.value?.querySelector(
    "#autocompleteInput",
  ) as HTMLInputElement;
  autocompleteDropdownArrow.value = shadowRoot.value?.querySelector(
    "#autocompleteDropdownArrow",
  ) as HTMLElement;
  autocompleteContainer.value = shadowRoot.value?.querySelector(
...
function inputHandler() {
  ...
  debounce = window.setTimeout(() => {
    if (autocompleteInput.value)
      searchFor.value = autocompleteInput.value.value.trim();
    debounce = null;
  }, 500);
}
...
function doSearch() {
  let language = (
    lang.value?.children[lang.value.selectedIndex] as HTMLOptionElement
  ).value;
