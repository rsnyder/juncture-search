...
const drawer = computed(
  () => shadowRoot.value?.querySelector(".drawer") as SLDrawer,
);
watch(drawer, (drawer) => {
  shadowRoot.value
...
watch(
  entitySelect,
  (entitySelect) =>
    entitySelect?.addEventListener("sl-change", (e: any) => setEntities(e)),
);
...
const providersEnabled = computed(() =>
  Object.fromEntries(
    (providers.value as any[])?.map((provider: any) => [
      provider.label,
      provider,
    ]) || [],
  ),
...
const depicted = computed(() =>
  Array.from(
    Object.entries(options.value?.depicts?.entities || {}).map(
      ([qid, count]) => ({ qid, count, label: labels.value[qid] }),
    ) || [],
  ),
);
...
function setProviders(e: any) {
  ...
  let provider = _options.providers.find(
    (p: any) => p.label === e.target.dataset.provider,
  );
  ...
  <div ref="root" class="relative">
    ...
    >
      <template v-for="(provider, idx) in providers">
        <div
          class="relative flex items-center py-2 px-3 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
        >
          <div class="flex items-center h-5">
            <input
              ...
              :data-enabled="
                providersEnabled[(provider as any).label]?.enabled
                  ? true
                  : false
              "
          ...
          <label
            :for="`hs-dropdown-item-checkbox-checkbox-${idx}`"
            class="ml-2"
          >
            <span
              class="text-sm font-semibold text-gray-800 dark:text-gray-300"
              v-html="(provider as any).label"
            ></span>
          </label>
        </div>
      </template>
      ...
      <sl-select
        ...
        clearable
      >
        <sl-option v-for="d in depicted" :key="d.qid" :value="d.qid"
          >{{ d.label }} ({{ d.count }})</sl-option
        >
      </sl-select>
