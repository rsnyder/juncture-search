      ...
      <div v-if="image" class="card">
        <div id="image">
          ...
          <div v-if="image.depicts">
            ...
            <ul>
              <li
                v-for="depict in image.depicts"
                :data-qid="depict.id"
                :key="depict.id"
              >
                <ve-entity-infobox
                  v-if="depict.label || labels[depict.id]"
                  :qid="depict.id"
                  v-html="depict.label || labels[depict.id]"
                ></ve-entity-infobox>
              </li>
            </ul>
          </div>
        ...
        <div id="metadata">
          <h2
            v-html="
              image.title ||
              image.file?.replace(/_/g, ' ').replace(/\.png|\.jpg|\.jpeg$/i, '')
            "
          ></h2>
          ...
          <div>
            ...
            <ul>
              <li>
                ...
                <a :href="image.url" v-html="image.url" target="_blank"></a>
              </li>
              <li>
                <span class="label">Thumbnail</span>
                <a
                  :href="image.thumbnail"
                  v-html="image.thumbnail"
                  target="_blank"
                ></a>
              </li>
              <li>
          ...
          <img
            class="h-6 w-6 cursor-copy"
            src="https://upload.wikimedia.org/wikipedia/commons/e/e8/International_Image_Interoperability_Framework_logo.png"
            @click="copyTextToClipboard(image.iiif)"
          />

          <div v-if="image.pageid">
            <span>ID</span>
            <a
              :href="`https://commons.wikimedia.org/wiki/Special:EntityData/M${image.pageid}.json`"
              v-html="`M${image.pageid}`"
              target="_blank"
            ></a>
          </div>
          ...
          <div v-if="image.width">
            <span>Size</span> {{ image.width?.toLocaleString() }} x
          ...
          <div v-if="image.format">
            <span>Format</span>
            <span v-html="image.format?.split('/').pop()?.toUpperCase()"></span>
          </div>
          ...
          <div v-if="image.imageQualityAssessment">
            ...
            <span v-html="image.imageQualityAssessment"></span>
          </div>
          ...
          <div>
            ...
            <a
              :href="image.license"
              v-html="licenses[image.license].code || image.license"
              target="_blank"
            ></a>
          </div>
...
function getImageInfo() {
  ...
  if (pageid) {
    fetch(
      `https://commons.wikimedia.org/w/api.php?origin=*&format=json&action=query&prop=imageinfo&iiprop=extmetadata|size|mime&pageids=${pageid}`,
    )
