        ...
        <th>
          <div class="prop-label">
            <a
              ...
              v-html="labels[pid]"
              :data-type="
                vals[0].mainsnak.snaktype == 'value'
                  ? vals[0].mainsnak.datavalue?.type
                  : ''
              "
              target="_blank"
            ></a>
        ...
        <td>
          <template v-if="entity.claims[pid].length > 5">
            <div class="collapsible">
              <input
                type="checkbox"
                name="collapse"
                :id="`${props.moduleId}-${pid}`"
              />

              <h2 class="handle">
                <label
                  :for="`${props.moduleId}-${pid}`"
                  v-html="
                    `${entity.claims[pid].length} ${
                      translations['items'] || 'items'
                    }.  ${translations['Show more'] || 'Show more'}.`
                  "
                ></label>
              </h2>
              ...
              <div class="prop-values-content">
                <div
                  v-for="(pval, idx) in entity.claims[pid]"
                  :key="`prop-${pid}-${idx}`"
                >
                  <!-- Property -->
                  <template v-if="pval.mainsnak.snaktype == 'value'">
                    <template
                      v-if="
                        pval.mainsnak.datavalue?.type === 'wikibase-entityid'
                      "
                    >
                      <!--
                      ...
                      <ve-entity-infobox
                        ...
                        v-html="
                          labels[pval.mainsnak.datavalue.value.id] ||
                          pval.mainsnak.datavalue.value.id
                        "
                      ></ve-entity-infobox>
                    ...
                    <span
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'wikibase-property'
                      "
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                    >
                      <a
                        :href="`http://www.wikidata.org/entity/${pval.mainsnak.datavalue.value.id}`"
                        target="_blank"
                        v-html="
                          `${labels[pval.mainsnak.datavalue.value.id]} (${
                            pval.mainsnak.datavalue.value.id
                    ...
                    <span
                      v-else-if="pval.mainsnak.datavalue?.type === 'quantity'"
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      v-html="quantity(pval.mainsnak.datavalue.value)"
                    ></span>
                    <span
                      v-else-if="pval.mainsnak.datavalue?.type === 'time'"
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      v-html="formattedTime(pval.mainsnak.datavalue.value)"
                    ></span>
                    <template
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'monolingualtext'
                      "
                    >
                      <span
                        v-if="
                          pval.mainsnak.datavalue.value.language === language
                        "
                        :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                        v-html="`${pval.mainsnak.datavalue.value.text}`"
                      ></span>
                    </template>
                    <template
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'commonsMedia' &&
                        commonsMediaImageMime(pval.mainsnak.datavalue.value)
                      "
                    >
                      <a
                        :href="`https://commons.wikimedia.org/wiki/File:${pval.mainsnak.datavalue.value}`"
                        target="_blank"
                      >
                        <img
                          :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                          :src="`https://commons.wikimedia.org/w/thumb.php?f=${encodeURIComponent(
                            pval.mainsnak.datavalue.value,
                          )}&w=200`"
                        />
                      </a>
                      <br /><a
                        :href="`https://commons.wikimedia.org/wiki/File:${pval.mainsnak.datavalue.value}`"
                        target="_blank"
                        v-html="pval.mainsnak.datavalue.value"
                      ></a>
                    </template>
                    <audio
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'commonsMedia' &&
                        commonsMediaAudioMime(pval.mainsnak.datavalue.value)
                      "
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      controls
                    >
                      <source
                        :src="commonsURL(pval.mainsnak.datavalue.value)"
                        :type="
                          commonsMediaAudioMime(pval.mainsnak.datavalue.value)
                        "
                      />
                    ...
                    <a
                      v-else-if="pval.mainsnak.datavalue?.type === 'url'"
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      :href="pval.mainsnak.datavalue.value"
                      v-html="pval.mainsnak.datavalue.value"
                      target="_blank"
                    ></a>
                    <a
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'external-id'
                      "
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      v-html="pval.mainsnak.datavalue.value"
                      :href="formatURL(pval.mainsnak.datavalue.value, pid)"
                      target="_blank"
                    ></a>
                    <a
                      v-else-if="
                        pval.mainsnak.datavalue?.type === 'globecoordinate'
                      "
                      :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                      v-html="coords(pval.mainsnak.datavalue.value)"
                      target="_blank"
                    ></a>
                    <span
                      v-else
                  ...
                  <!-- qualifiers -->
                  <div
                    v-if="pval.qualifiers"
                    class="qualifier"
                    v-for="(values, pid) in pval.qualifiers"
                    :key="`qual-${pid}`"
                  >
                    <a
                      class="prop-label"
                      v-html="labels[pid] || pid"
                      :href="`https://www.wikidata.org/wiki/Property:${pid}`"
                    ></a
                    >:
                    <template v-for="(val, vidx) in values">
                      <template v-if="val.snaktype == 'value'">
                        <template
                          v-if="val.datavalue?.type === 'wikibase-entityid'"
                        >
                          <!--
                          ...
                          <ve-entity-infobox
                            ...
                            v-html="
                              labels[val.datavalue.value.id] ||
                              val.datavalue.value.id
                            "
                          ></ve-entity-infobox>
                        </template>
                        <span
                          v-else-if="val.datavalue?.type === 'quantity'"
                          :class="`prop-value ${val.datavalue?.type}`"
                          v-html="quantity(val.datavalue.value)"
                        ></span>
                        <span
                          v-else-if="val.datavalue?.type === 'time'"
                          :class="`prop-value ${val.datavalue?.type}`"
                          v-html="formattedTime(val.datavalue.value)"
                        ></span>
                        <template
                          v-else-if="val.datavalue?.type === 'monolingualtext'"
                        >
                          <span
                            v-if="val.datavalue.value.language === language"
                            :class="`prop-value ${val.datavalue?.type}`"
                            v-html="`${val.datavalue.value.text}`"
                          ></span>
                        </template>
                        <img
                          v-else-if="
                            val.datavalue?.type === 'commonsMedia' &&
                            commonsMediaImageMime(val.datavalue.value)
                          "
                          :class="`prop-value ${val.datavalue?.type}`"
                          :src="`https://commons.wikimedia.org/w/thumb.php?f=${encodeURIComponent(
                            val.datavalue.value,
                        ...
                        <audio
                          v-else-if="
                            val.datavalue?.type === 'commonsMedia' &&
                            commonsMediaAudioMime(val.datavalue.value)
                          "
                          :class="`prop-value ${val.datavalue?.type}`"
                          controls
                        >
                          <source
                            :src="commonsURL(val.datavalue.value)"
                            :type="commonsMediaAudioMime(val.datavalue.value)"
                          />
                        </audio>
                        <a
                          v-else-if="val.datavalue?.type === 'url'"
                          :class="`prop-value ${val.datavalue?.type}`"
                          :href="val.datavalue.value"
                          v-html="val.datavalue.value"
                          target="_blank"
                        ></a>
                        <a
                          v-else-if="val.datavalue?.type === 'external-id'"
                          :class="`prop-value ${val.datavalue?.type}`"
                          v-html="val.datavalue.value"
                          :href="formatURL(val.datavalue.value, pid)"
                          target="_blank"
                        ></a>
                        <a
                          v-else-if="val.datavalue?.type === 'globecoordinate'"
                          :class="`prop-value ${val.datavalue?.type}`"
                          v-html="coords(val.datavalue.value)"
                          target="_blank"
                        ></a>
                        <span
                          v-else
                  ...
                  <div v-if="pval.references" class="references collapsible">
                    <input
                      type="checkbox"
                      name="collapse"
                      :id="`${props.moduleId}-${pid}-${idx}`"
                    />
                    <h2 class="handle">
                      <label
                        :for="`${props.moduleId}-${pid}-${idx}`"
                        v-html="
                          `${pval.references.length} ${
                            pval.references.length === 1
                    ...
                    <div class="ref-content">
                      <div
                        class="reference"
                        v-for="(reference, ridx) in pval.references"
                        :key="`rf${ridx}`"
                      >
                        <div
                          v-for="(values, pid, pidx) in reference.snaks"
                          :key="`r${ridx}-${pidx}`"
                        >
                          <a
                            class="prop-label"
                          ...
                          <template v-for="(val, idx) in values">
                            <template v-if="val.snaktype == 'value'">
                              <template
                                v-if="
                                  val.datavalue?.type === 'wikibase-entityid'
                                "
                              >
                                <!--
                              ...
                              <template
                                v-else-if="
                                  val.datavalue?.type === 'monolingualtext'
                                "
                              >
                                <span
                                  v-if="
                                    val.datavalue.value.language === language
                                  "
                                  :class="`prop-value ${val.datavalue?.type}`"
                                  v-html="`${val.datavalue.value.text}`"
                                ></span>
                              ...
                              <a
                                v-else-if="
                                  val.datavalue?.type === 'external-id'
                                "
                                :class="`prop-value ${val.datavalue?.type}`"
                                v-html="val.datavalue.value"
                                :href="formatURL(val.datavalue.value, pid)"
                              ...
                              <a
                                v-else-if="
                                  val.datavalue?.type === 'globecoordinate'
                                "
                                :class="`prop-value ${val.datavalue?.type}`"
                                v-html="coords(val.datavalue.value)"
                                target="_blank"
          ...
          <template v-else>
            ...
            >
              ...
              <template v-if="pval.mainsnak.snaktype == 'value'">
                <template
                  v-if="pval.mainsnak.datavalue?.type === 'wikibase-entityid'"
                >
                  <!--
                ...
                >
                  <span
                    v-if="pval.mainsnak.datavalue.value.language === language"
                    :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                    v-html="`${pval.mainsnak.datavalue.value.text}`"
                  ></span>
                </template>
                ...
                >
                  <source
                    :src="commonsURL(pval.mainsnak.datavalue.value)"
                    :type="commonsMediaAudioMime(pval.mainsnak.datavalue.value)"
                  />
                </audio>
                ...
                <a
                  v-else-if="pval.mainsnak.datavalue?.type === 'external-id'"
                  :class="`prop-value ${pval.mainsnak.datavalue?.type}`"
                  v-html="pval.mainsnak.datavalue.value"
                  :href="formatURL(pval.mainsnak.datavalue.value, pid)"
                  target="_blank"
...
const imageMimeTypes: Dictionary<string> = {
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
  png: "image/png",
  gif: "image/gif",
...
const audioMimeTypes: Dictionary<string> = {
  ogg: "audio/ogg",
  wav: "audio/wav",
  mp3: "audio/mpeg",
};
  ...
  return _claims &&
    ...
    ? Object.fromEntries(
        Object.keys(_claims)
          ...
          .filter(
            (pid: string) =>
              _claims[pid][0].mainsnak.datatype !== "external-id",
          )
...
function formatURL(val: any, pid: any): string {
  return urlformatters.value[pid]
    ? urlformatters.value[pid].replace(/\$1/, val)
    : val;
}
...
function commonsURL(val: string) {
  ...
  return `https://upload.wikimedia.org/wikipedia/commons/${_md5.slice(
    0,
    1,
  )}/${_md5.slice(0, 2)}/${val}`;
}
