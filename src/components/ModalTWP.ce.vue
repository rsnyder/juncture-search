...
watch(modal, () => {
  ...
  modal.value.addEventListener(
    "open.hs.overlay",
    (e: any) => (isOpen.value = true),
  );
  modal.value.addEventListener(
    "close.hs.overlay",
    (e: any) => (isOpen.value = false),
  );
});
...
<template>
  <div ref="overlayRef">
    ...
    >
      ...
      >
        <div
          class="w-full max-h-full flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]"
        >
          <div
