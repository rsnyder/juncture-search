<template>
  <ve-header
    class="max-w-5xl m-auto rounded mb-4"
    color="#444A1E"
    logo="/images/phl-logo.png"
    url="/"
  ...
  <div class="max-w-5xl m-auto">
    <panel>
      <h1 class="text-2xl font-bold mb-4">
        Welcome to the Plant Humanities Search tool
      </h1>
      <p>
        This search tool provides a convenient way to find web resources for a
        named entity (a person, place, plant, etc) using the Wikidata knowledge
        graph - for use in creating visual essays, teaching, and research.
      </p>
      <p class="mt-4">Entity search results include:</p>
      <ul class="ml-4 max-w-lg list-disc list-inside dark:text-gray-400">
        <li>High resolution images (public domain and openly licensed)</li>
    ...
    <panel>
      ...
      <p>
        An entity is anything that can be considered, discussed, or observed.
        Common examples include people, places, organizations, plants, animals,
        and events. The Plant Humanities Search tool uses the
        <a href="https://www.wikidata.org/">Wikidata</a> knowledge graph to
        extract information about entities, and to find related web resources.
      </p>
    </panel>
    ...
    <panel class="bg-[#a2a58f]">
      <div class="flex-col w-full max-w-[500px] m-auto">
        ...
        <p class="mt-2">
          Search the Wikidata knowledge graph for a person, place, plant,
          animal, etc.
        </p>
    ...
    <panel>
      <h1 class="text-2xl font-bold">Contribute</h1>
      <p>
        This search tool uses returns web resources that have been tagged with
        with an explicit relationship to the entity of interest and others where
        the relationship is inferred, often using keyword search which can be
        imprecise. Users of this tool can help improve the quality of the
        results. Learn more in the
        <a href="/about#contributing">contributing section on our About page</a>
      </p>
    </panel>
  </div>
</template>
...
watch(qid, () => {
  if (qid.value) setRoute(qid.value, language.value);
});
watch(language, () => {
  if (qid.value) setRoute(qid.value, language.value);
});
