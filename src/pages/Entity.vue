...
store.setQid(
  Array.isArray(route.params.qid) ? route.params.qid[0] : route.params.qid,
);
store.setLanguage(
...
watch(route, () => {
  let qid = Array.isArray(route.params.qid)
    ? route.params.qid[0]
    : route.params.qid;
  let language =
    (Array.isArray(route.query.lang)
      ? route.query.lang[0]
      : route.query.lang) || "en";
  store.setQid(qid);
...
watch(qid, () => {
  if (qid.value) setRoute(qid.value, language.value, active.value);
});
watch(language, () => {
  if (qid.value) setRoute(qid.value, language.value, active.value);
});
