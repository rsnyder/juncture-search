...
export async function handler(event) {
  ...
  while (categoryImagesContinue !== undefined) {
    let resp = await fetch(
      cookieJar,
      `https://commons.wikimedia.org/w/api.php?origin=*&action=query&generator=categorymembers&gcmlimit=500&gcmtitle=Category:${category.replace(
        / /,
        "_",
      )}&format=json&gcmnamespace=6&gcmtype=file&prop=imageinfo&iiprop=url&gcmcontinue=${categoryImagesContinue}`,
      {
